			...
			void ZebraAnalysisView::regenerate()
			{
				...
				if (selector_->isMatchesSelected())
				{
					...
					for (int slot = 1; slot <= 3; slot++)
					{
						...
						auto track = DataModelBuilder::createTrack(dataModel(), key, tkey, 54.0, 27.0);
						if (track->hasData())
				...
				else
				{
					for (auto m : dataModel()->matches())
					{
						...
						{						
							auto track = DataModelBuilder::createTrack(dataModel(), m->key(), key, 54.0, 27.0);
							if (track->hasData())
				...
				analyzer_.analyze(selected, 54.0, 27.0);
				QString html;
